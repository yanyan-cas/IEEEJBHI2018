...
\author{Yan~Yan,~\IEEEmembership{Student Member,~IEEE,}
       % x~x,~\IEEEmembership{Senior Member,~IEEE,}
        and~Lei~Wang,~\IEEEmembership{Member,~IEEE}% <-this % stops a space
...
\begin{abstract}
Exploring the low-dimensional embedding from high-dimensional electrocardiography data set allows investigation of the data structure, which helps to analyze the cardiac electrophysiological activity.
In this paper, we tested several existing manifold learning techniques for discovering such embeddings within the electrophysiological activity space for clinical heart monitoring and arrhythmia analysis.
Among the existing manifold learning methods, the linear methods, such as principal component analysis; nonlinear local-based methods, such as xxx; and nonlinear global-based methods, such as Isomap and kernel PCA are explored in arrhythmia analysis.
Based on the manifold learning techniques we constructed a ensemble manifold learning framework, which overcame the single manifold learning weakness in the classification problem.
The framework meets both the diversity and accuracy criteria requirements in the ensemble learning technique, which improved the performance of arrhythmia analysis.

The proposed framework provide a novel way exploring low-dimensional embeddings, and a decent results were achieved in the electrocardiography analysis tasks.
The data sets for this study comes from several data sets from Physionet.
We illustrated the intrinsic dimension analysis results and imbalance analysis for this specific task of arrhythmia analysis.
With different choice of manifolds, we explore different combinations for this ensemble manifold learning framework.
\end{abstract}
